    ...
    public class StartUp:StartUpTrans
    {
        ...
        public override void Run()
        {
           Namespace = "CACTPC1";
            try
            {
                ...
                if (CommandInfo == null)
                {
                    Sm.Windows.Controls.ExMessageBox.Show( 850,StartUp.SysObj, "Chưa khai báo command hoặc command ngầm định sai!", "", MessageBoxButton.OK, MessageBoxImage.Information);
                ...
                if (DmctInfo == null)
                {
                    Sm.Windows.Controls.ExMessageBox.Show( 855,StartUp.SysObj, "Chưa khai báo chứng từ hoặc chứng từ ngầm định sai!", "", MessageBoxButton.OK, MessageBoxImage.Information);
                    if (!Process.GetCurrentProcess().ProcessName.Equals("SmProcess"))
                        App.Current.Shutdown();
                    return;
                }
                ...
                FrmCACTPC1 _Form = new FrmCACTPC1();
                ...
                if (Editing_Stt_Rec.Equals(string.Empty) && M_CHK_DATE_YN == "1")
                {
                    ...
                    dlgSelTime.LanguageID = "CACTPC1SelTime";
                ...
                _Form.Title = SmLib.SysFunc.Cat_Dau(M_LAN.Equals("V") ? StartUp.CommandInfo["bar"].ToString() : StartUp.CommandInfo["bar2"].ToString());
        ...
        public static void DeleteVoucher(string _stt_rec)
        {
            try
            {
                ...
                //cmd = new SqlCommand("exec [dbo].[CACTPC1-UpdateTt] @stt_rec");
                //cmd.Parameters.Add("@stt_rec", SqlDbType.Char, 11).Value = _stt_rec;
                //StartUp.SysObj.ExcuteNonQuery(cmd);
            }
            catch (Exception ex)
        ...
        {
            SqlCommand cmd = new SqlCommand("exec [dbo].[CACTPC1-CheckData] @status, @stt_rec");
