...
namespace dmkx
{
    ...
    public partial class newForm : FormList
    {
        ...
        void LoadForm()
        {
            ...
            txtma_kx.MaxLength = ListFunc.GetLengthColumn(TableFields, "ma_kx");
            txtten_kx.MaxLength = ListFunc.GetLengthColumn(TableFields, "ten_kx");
            txtten_kx2.MaxLength = ListFunc.GetLengthColumn(TableFields, "ten_kx");
                        ...
                        try
                        {
                            ...
                            if (StartUp.SysObj.GetOption("M_AUTO_LIST_NUM").ToString().Equals("1"))
                            {
                                if (!string.IsNullOrEmpty(StartUp.currSqlTableKey) && StartUp.SysObj.DmdmInfo.Select("ma_dm like '" + StartUp.sqlTableName + "' and  increase_type = 2").Length > 0)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(_value) && _value.Length <= txtma_kx.MaxLength)
        ...
        {
            bool result = true;
            if (txtma_kx.Text.Trim() == string.Empty && result == true)
            {
            ...
            if (txtma_kx.Text.Trim() != "" && result == true)
            {
                string sb = SmLib.SysFunc.CheckInValidCode(StartUp.SysObj, txtma_kx.Text.Trim());
                if (sb != "" && result == true)
                {
                    TabInfor.SelectedIndex = 0;
                ...
                if (result == true)
                {
                    try
                    {
                        ...
                        cmdGet.Parameters.Add("@ma_dm", SqlDbType.Char).Value = StartUp.sqlTableName;
                        cmdGet.Parameters.Add("@" + StartUp.SqlTableKey, SqlDbType.Char).Value = txtma_kx.Text.Trim();
                        ...
                        if (result == true && ischeck == true)
                        {
                            ...
                            if (SmLib.SysFunc.CheckStringContain(StartUp.SysObj, StartUp.sqlTableName, StartUp.SqlTableKey, txtma_kx.Text.Trim(), ma_cu))
                            {
                                TabInfor.SelectedIndex = 0;
            ...
            if (txtten_kx.Text.Trim() == string.Empty && result == true)
            {
                TabInfor.SelectedIndex = 0;
        ...
        #region txtma_kx_GotFocus
        private void txtma_kx_GotFocus(object sender, RoutedEventArgs e)
        {
            txtma_kx.Text = txtma_kx.Text.Trim();
            txtma_kx.SelectAll();
        }
        #endregion
