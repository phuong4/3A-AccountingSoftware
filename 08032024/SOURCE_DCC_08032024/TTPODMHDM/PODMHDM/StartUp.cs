...
namespace TTPODMHDM
{
    public class StartUp : StartUpTrans
    {
        ...
        public override void Run()
        {
           Namespace = "TTPODMHDM";
            try
            {
                ...
                if (CommandInfo == null)
                {
                    ExMessageBox.Show( 1360,StartUp.SysObj, "Chưa khai báo command hoặc command ngầm định sai!", "", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                if (DmctInfo == null)
                {
                    ExMessageBox.Show( 1365,StartUp.SysObj, "Chưa khai báo chứng từ hoặc chứng từ ngầm dịnh sai!", "Fast Book 11 .NET", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                ...
                if (Editing_Stt_Rec.Equals(string.Empty) && M_CHK_DATE_YN == "1")
                {
                    ...
                    dlgSelTime.LanguageID = "PODMHDMSelTime";
                ...
                TransFilterCmd.Parameters.Add("@PhFilter", SqlDbType.NVarChar, 4000).Value = (Editing_Stt_Rec.Equals(string.Empty) ? "1=1" + " AND ma_dvcs = '" + SysObj.M_ma_dvcs.Trim() + "' AND rtrim(ltrim(ma_ct)) = '" + Ma_ct.Trim() + "'" : "stt_rec = '" + Editing_Stt_Rec + "'  AND rtrim(ltrim(ma_ct)) = '" + Ma_ct.Trim() + "'") + conditionPH;
                TransFilterCmd.Parameters.Add("@CtFilter", SqlDbType.NVarChar, 4000).Value = " rtrim(ltrim(ma_ct)) = '" + Ma_ct.Trim() + "'";
        ...
        {
            SqlCommand cmd = new SqlCommand("exec [dbo].[PODMHDM-CheckData] @status, @stt_rec");
