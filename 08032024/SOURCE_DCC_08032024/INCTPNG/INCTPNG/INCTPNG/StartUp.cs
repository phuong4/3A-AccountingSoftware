...
namespace Inctpng
{
    public class StartUp : StartUpTrans
    {
        ...
        //Dùng trong form tìm
        static public string tableList = "v_ph77;v_ct77;v_ct77gt";
        ...
        public override void Run()
        {
           Namespace = "Inctpng";
            try
            {
                ...
                FrmInctpng _Form = new FrmInctpng();
                ...
                if (CommandInfo == null)
                {
                    ExMessageBox.Show( 840,StartUp.SysObj, "Chưa khai báo command hoặc command ngầm định sai!", "", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                if (DmctInfo == null)
                {
                    ExMessageBox.Show( 845,StartUp.SysObj, "Chưa khai báo chứng từ hoặc chứng từ ngầm dịnh sai!", "Fast Book 11 .NET", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                ...
                if (Editing_Stt_Rec.Equals(string.Empty) && M_CHK_DATE_YN == "1")
                {
                    ...
                    dlgSelTime.LanguageID = "InctpngSelTime";
        ...
        public static void DeleteVoucher(string _stt_rec)
        {
            try
            {
                ...
                cmd = new SqlCommand("[dbo].[DeleteVoucher]");
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@cMa_ct", SqlDbType.Char, 3).Value = "PXG";
                cmd.Parameters.Add("@stt_rec", SqlDbType.Char, 11).Value = _stt_rec.Replace("PNG", "PXG");
                StartUp.SysObj.ExcuteNonQuery(cmd);
            }
            catch (Exception ex)
            {
        ...
        public static void GetDmnt(DataSet ds)
        {
        ...
        {
            SqlCommand cmd = new SqlCommand("exec [dbo].[Inctpng-CheckData] @status, @stt_rec");
