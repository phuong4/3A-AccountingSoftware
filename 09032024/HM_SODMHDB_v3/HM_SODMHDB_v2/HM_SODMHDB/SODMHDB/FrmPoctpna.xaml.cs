...
namespace AAA_SODMHDB
{
    ...
    public partial class FrmPoctpna : SmVoucherLib.FormTrans
    {
                    ...
                    if (StartUp.M_NHIEU_VAT_HDA == "0")
                    {
                        ...
                        if (!isError)
                        {
                            ...
                            ThreadStart _thread = delegate ()
                            {
                                ...
                                if (!string.IsNullOrEmpty(StartUp.DsTrans.Tables[0].DefaultView[0]["ngay_ra"].ToString()) && string.IsNullOrEmpty(StartUp.DsTrans.Tables[0].DefaultView[0]["tiencou_nt"].ToString()))
                                {
                                ...
                                else
                                {
                                    ...
                                    Dispatcher.BeginInvoke(DispatcherPriority.Normal,
                                               new Action(() =>
                                               {
                                                   MessageBox.Show("Tạo hoá đơn bảo hiểm thành công!");
                                               }));
        ...
        void Sum_ALL()
        {
            ...
            decimal t_tiencou_nt = ParseDecimal(StartUp.DsTrans.Tables[0].Rows[iRow]["tiencou_nt"], 0);
        ...
        {
            SqlCommand PostCmd = new SqlCommand("exec [AAA_SODMHDB-HDA] @stt_rec,@stt_rec_HDA");
            PostCmd.Parameters.Add("@stt_rec", SqlDbType.VarChar).Value = StartUp.DsTrans.Tables[0].DefaultView[0]["stt_rec"].ToString();
        ...
        {
            SqlCommand PostCmd = new SqlCommand("exec [AAA_SODMHDB-HDA2] @stt_rec,@stt_rec_HDA");
            PostCmd.Parameters.Add("@stt_rec", SqlDbType.VarChar).Value = StartUp.DsTrans.Tables[0].DefaultView[0]["stt_rec"].ToString();
            PostCmd.Parameters.Add("@stt_rec_HDA", SqlDbType.VarChar).Value = @stt_rec_hda;
            StartUp.SysObj.ExcuteNonQuery(PostCmd);
        ...
        {
            SqlCommand PostCmd = new SqlCommand("exec [AAA_SODMHDB-HDA3] @stt_rec,@stt_rec_HDA");
            PostCmd.Parameters.Add("@stt_rec", SqlDbType.VarChar).Value = StartUp.DsTrans.Tables[0].DefaultView[0]["stt_rec"].ToString();
            PostCmd.Parameters.Add("@stt_rec_HDA", SqlDbType.VarChar).Value = @stt_rec_hda;
            StartUp.SysObj.ExcuteNonQuery(PostCmd);
            SqlCommand cmd = new SqlCommand("SELECT stt_rec_hd from dmhd where stt_rec=@sttrec");
        ...
        private void btnOk_Click(object sender, RoutedEventArgs e)
        {
            ...
            for (int i = 0; i < count_ct; i++)
            {
                if (StartUp.DsTrans.Tables[1].DefaultView[i]["ma_vt"].ToString().Trim() == txtMa_vt2.Text.Trim())
                {
                    ...
                    StartUp.DsTrans.Tables[1].DefaultView[i]["thue_nt"] = SysFunc.Round((tien_nt2 * 8 / 100), 0);
                    StartUp.DsTrans.Tables[1].DefaultView[i]["thue"] = SysFunc.Round((tien_nt2 * 8 / 100), 0);
