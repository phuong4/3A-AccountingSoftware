...
namespace AAA_SODMHDB
{
    public class StartUp : StartUpTrans
    {
        ...
        public static string M_MA_BIN = "";
        public static string SO_TAI_KHOAN = "";

        static public string M_BP_BH = string.Empty;

        static public string is_admin = "";
        //Dùng trong form tìm
        static public string tableList = "v_DMHD;v_DMHDCT";
        ...
        public override void Run()
        {
           Namespace = "AAA_SODMHDB";
            try
            {
                ...
                M_NHIEU_VAT_HDA = SysObj.GetOption("M_NHIEU_VAT_HDA").ToString().Trim();
                ...
                M_MA_BIN = SysObj.GetOption("M_MA_BIN").ToString();
                SqlCommand strmabin = new SqlCommand("SELECT ltrim(rtrim(ma_bin)) ma_bin, so_tai_khoan " +
                "FROM [dmbin] WHERE ma_bin = @ma_bin");
                strmabin.Parameters.Add("@ma_bin", SqlDbType.Char).Value = M_MA_BIN;


                DataTable tblbin = StartUp.SysObj.ExcuteReader(strmabin).Tables[0];
                SO_TAI_KHOAN = tblbin.Rows[0]["so_tai_khoan"].ToString();
                ...
                if (CommandInfo == null)
                {
                    ExMessageBox.Show( 2360,StartUp.SysObj, "Chưa khai báo command hoặc command ngầm định sai!", "", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                if (DmctInfo == null)
                {
                    ExMessageBox.Show( 2365,StartUp.SysObj, "Chưa khai báo chứng từ hoặc chứng từ ngầm dịnh sai!", "Fast Book 11 .NET", MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }
                ...
                is_admin = StartupBase.SysObj.UserInfo.Rows[0]["is_admin"].ToString().Trim();
                ...
                if (Editing_Stt_Rec.Equals(string.Empty) && M_CHK_DATE_YN == "1")
                {
                    ...
                    dlgSelTime.LanguageID = "AAA_SODMHDBSelTime";
        ...
        {
            SqlCommand cmd = new SqlCommand("exec [dbo].[AAA_SODMHDB-CheckData] @status, @stt_rec,@ma_gd");
            ...
            cmd.Parameters.Add("@ma_gd", SqlDbType.Char, 1).Value = StartUp.DsTrans.Tables[0].DefaultView[0]["ma_gd"];
            return StartUp.SysObj.ExcuteReader(cmd);
        }
        //public static DataTable GetCtadd(string stt_recin)
        //{
        //    SqlCommand sqlCommand = new SqlCommand("EXEC dbo.AA_Getctin @stt_rec,@ma_dvcs");
        //    sqlCommand.Parameters.Add("@Ma_sp", SqlDbType.Char).Value = stt_recin;
