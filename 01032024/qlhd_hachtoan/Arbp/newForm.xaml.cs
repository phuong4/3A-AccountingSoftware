...
namespace qlhd_hachtoan
{
    ...
    public partial class newForm : FormList
    {
        ...
        void LoadForm()
        {
            ...
            //txtMa_ct.MaxLength = ListFunc.GetLengthColumn(TableFields, "ma_ct");
            //txtten_bp.MaxLength = ListFunc.GetLengthColumn(TableFields, "ten_bp");
            //txtten_bp2.MaxLength = ListFunc.GetLengthColumn(TableFields, "ten_bp");
                ...
                case ActionTask.Add:
                    {
                        try
                        {
                            ...
                            if (StartUp.SysObj.GetOption("M_AUTO_LIST_NUM").ToString().Equals("1"))
                            {
                                ...
                                //    if (!string.IsNullOrEmpty(_value) && _value.Length <= txtMa_ct.MaxLength)
                ...
                case ActionTask.Edit:
                    {
                        try
                        {
                            ...
                            if (newDataTable.Rows.Count > 0)
                            {
                                ...
                                //Thêm điều kiện lọc khi chọn mã chứng từ
                                txtMa_qs.Filter = "ma_cts like '%" + OldRow.Rows[0]["ma_ct"].ToString().Trim() + "%' and status=1";
                            }
        ...
        {
            bool result = true;
            if (txtMa_ct.Text.Trim() == string.Empty && result == true)
            {
                TabInfor.SelectedIndex = 0;
                ExMessageBox.Show(15, StartUp.SysObj, "Chưa vào mã chứng từ!", "Xac nhan nhap lieu", MessageBoxButton.OK, MessageBoxImage.Information);

                txtMa_ct.Focus();
                result = false;
            }
            if (txtMa_ct.Text.Trim() != "" && result == true)
            {
                //string sb = SmLib.SysFunc.CheckInValidCode(StartUp.SysObj, txtMa_ct.Text.Trim());
                //if (sb != "" && result == true)
                //{
                //    TabInfor.SelectedIndex = 0;
                ...
                if (result == true)
                {
                    try
                    {
                        ...
                        cmdGet.Parameters.Add("@ma_dm", SqlDbType.Char).Value = StartUp.sqlTableName;
                        cmdGet.Parameters.Add("@" + StartUp.SqlTableKey, SqlDbType.Char).Value = txtMa_ct.Text.Trim();
                        ...
                        if (result == true && ischeck == true)
                        {
                            ...
                            if (SmLib.SysFunc.CheckStringContain(StartUp.SysObj, StartUp.sqlTableName, StartUp.SqlTableKey, txtMa_ct.Text.Trim(), ma_cu))
                            {
                                TabInfor.SelectedIndex = 0;
            ...
            //if (txtten_bp.Text.Trim() == string.Empty && result == true)
            //{
            //    TabInfor.SelectedIndex = 0;
        ...
        //{
        //    txtMa_ct.Text = txtMa_ct.Text.Trim();
        //    txtMa_ct.SelectAll();
        //}
        #endregion
        ...
        private void txtMa_ct_LostFocus(object sender, RoutedEventArgs e)
        {
            if(txtMa_ct.RowResult == null)
                txtten_ct.Text = "";
            else
                txtten_ct.Text = StartUp.M_LAN.Equals("V") ? txtMa_ct.RowResult["ten_ct"].ToString() : txtMa_ct.RowResult["ten_ct2"].ToString();
        }

        private void Chkcheck_group_detail_Click(object sender, RoutedEventArgs e)
        {
