...
namespace AA_BC05
{
	public partial class FrmTaoPX : Form
	{
		...
		private void ConfirmGridView_OnOk(object sender, RoutedEventArgs e)
		{
			...
			{
				ExMessageBox.Show(697, StartupBase.SysObj, "Chưa vào mã quyển sổ phiếu xuất", "", MessageBoxButton.OK, MessageBoxImage.Asterisk);
				this.txtMa_qs_px.IsFocus = true;
			}
			else if (string.IsNullOrEmpty(this.txtso_ct_px.Text.Trim()))
			{
				ExMessageBox.Show(698, StartupBase.SysObj, "Chưa vào số chứng từ!", "Xac nhan nhap lieu", MessageBoxButton.OK, MessageBoxImage.Asterisk);
				this.txtso_ct_px.Focus();
			}
			else if (string.IsNullOrEmpty(this.txtMa_kh.Text.Trim()))
			{
				ExMessageBox.Show(694, StartupBase.SysObj, "Chưa vào mã khách!", "Xac nhan nhap lieu", MessageBoxButton.OK, MessageBoxImage.Asterisk);
			...
			{
				ExMessageBox.Show(692, StartupBase.SysObj, "Chưa vào tài khoản nợ!", "Xac nhan nhap lieu", MessageBoxButton.OK, MessageBoxImage.Asterisk);
		...
		private void Form_Loaded(object sender, RoutedEventArgs e)
		{
			...
			if (flag)
			{
				this.txtMa_gd.Filter = "ma_ct = 'PXD' and status = 1 and ma_gd IN ('4')";
			...
			else
			{
				...
				this.txtMa_gd.Filter = "ma_ct = 'PXD' and status = 1 and ma_gd IN ('4')";
				sqlCommand.CommandText = string.Format(sqlCommand.CommandText, "PXD", "PXD");
