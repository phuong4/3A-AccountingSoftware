...
namespace AA_SmImexCt0
{
    ...
    public partial class newForm : FormList
    {
        ...
        void LoadForm()
        {
            ...
            txtma_bp.MaxLength = ListFunc.GetLengthColumn(TableFields, "ma_bp");
                        ...
                        try
                        {
                            ...
                            if (StartUp.SysObj.GetOption("M_AUTO_LIST_NUM").ToString().Equals("1"))
                            {
                                if (!string.IsNullOrEmpty(StartUp.currSqlTableKey) && StartUp.SysObj.DmdmInfo.Select("ma_dm like '" + StartUp.sqlTableName + "' and  increase_type = 2").Length > 0)
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(_value) && _value.Length <= txtma_bp.MaxLength)
        ...
        {
            bool result = true;
            if (txtma_bp.Text.Trim() == string.Empty && result == true)
            {
            ...
            if (txtma_bp.Text.Trim() != "" && result == true)
            {
                string sb = SmLib.SysFunc.CheckInValidCode(StartUp.SysObj, txtma_bp.Text.Trim());
                if (sb != "" && result == true)
                {
                    TabInfor.SelectedIndex = 0;
                ...
                if (result == true)
                {
                    try
                    {
                        ...
                        cmdGet.Parameters.Add("@ma_dm", SqlDbType.Char).Value = StartUp.sqlTableName;
                        cmdGet.Parameters.Add("@" + StartUp.SqlTableKey, SqlDbType.Char).Value = txtma_bp.Text.Trim();
                        ...
                        if (result == true && ischeck == true)
                        {
                            ...
                            if (SmLib.SysFunc.CheckStringContain(StartUp.SysObj, StartUp.sqlTableName, StartUp.SqlTableKey, txtma_bp.Text.Trim(), ma_cu))
                            {
                                TabInfor.SelectedIndex = 0;
        ...
        {
            txtma_bp.Text = txtma_bp.Text.Trim();
            txtma_bp.SelectAll();
        }
        #endregion
