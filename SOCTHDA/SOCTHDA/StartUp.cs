    ...
    public class StartUp:StartUpTrans
    {
        ...
        public override void Run()
        {
           Namespace = "Socthda";
            ...
            if (Editing_Stt_Rec.Equals(string.Empty) && M_CHK_DATE_YN == "1")
            {
                ...
                dlgSelTime.LanguageID = "SOCTHDASelTime";
     ...
     _Form.Title = M_Tilte;
        ...
        public static string GetLanguageString(string code, string language)
        {
            if (code == "M_MA_NT")
            {
                if (StartUp.DsTrans.Tables[0].Rows[FrmSocthda.iRow]["ma_nt"].ToString().Equals(StartUp.M_ma_nt0))
                    return "";
                else
                    return StartUp.DsTrans.Tables[0].Rows[FrmSocthda.iRow]["ma_nt"].ToString();
            }
            if (code == "M_MA_NT0")
